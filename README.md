...
Minimum recommended hardware: Raspberry Pi <2/Zero 2, Orange Pi Zero 2, or Makerbase MKS PI
...
Install Raspberry Pi Imager for Linux (Flatpak, distro, [direct), Windows, or macOS](https://www.raspberrypi.com/software/)
...
The TMC2225 datasheet has a box at the top of page 16 that says, "The UART line must be logic high during idle state..." The KP3S does not ship with stepper motor controllers' UART mode enabled. There is a 20k resistor (marked "30C") for the TMC2225 pin 17 (PDN_UART) that is connected to ground. This resistor must be removed to enable UART mode. The picture below highlights these resistor for each driver. There is a solder jumper pad near each of the drivers that needs to be bridged to enable UART mode. The following is a list of each driver, the pin number of the micro controller and the port number for that pin.
